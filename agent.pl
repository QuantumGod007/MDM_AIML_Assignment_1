...
remedy(leaf_blight,    'Apply copper-based fungicide and remove infected leaves.').
remedy(powdery_mildew, 'Apply sulfur spray and improve air circulation.').
remedy(root_rot,       'Improve soil drainage and reduce watering frequency.').
remedy(mosaic_virus,   'Remove infected plants and control aphid population.').
...
match_count(Disease, Count) :-
    ...
    findall(S, (member(S, Symptoms), patient_symptom(S)), Matched),
...
diagnose(BestDisease) :-
    findall(Count-Disease, candidate(Disease, Count), Pairs),
    Pairs \= [],
...
perform(no_match) :-
    format("No matching disease found for given symptoms. Consult an expert.~n", []).

start :-
    ( diagnose(_) -> perform(diagnose) ; perform(no_match) ).
...
add_symptom(S) :-
    ( patient_symptom(S) -> true ; assert(patient_symptom(S)) ),
